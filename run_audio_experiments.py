...
def run_experiment(fraction):
    print(f"\n1. Creating Project for {fraction*100}% Audio Scale...")
    proj_resp = requests.post(f"{BASE_URL}/projects/", json={"name": f"Phase 9 Audio", "description": "Scaling experiments"})
    proj_id = proj_resp.json().get("id") or 1 # Fallback if already exists
    
    print(f"2. Creating Experiment in Project {proj_id}...")
    exp_resp = requests.post(f"{BASE_URL}/projects/{proj_id}/experiments/", json={"name": f"Audio-Scale-{fraction*100}pct"})
