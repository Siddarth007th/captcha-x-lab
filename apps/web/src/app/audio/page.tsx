...
import { Mic, Waves, ActivitySquare, Zap, AlertTriangle } from "lucide-react";
import { CustomBarChart, CustomLineChart } from "../../components/Charts";
import { FailureList } from "../../components/FailureList";

export default async function AudioPage() {
  const projects = await fetchProjects();
  let audioExperiments: any[] = [];
  
  for (const p of projects) {
    const exps = await fetchExperiments(p.id);
    const aExps = exps.filter(e => e.name.toLowerCase().includes('audio') || e.name.toLowerCase().includes('wav2vec') || e.name.toLowerCase().includes('speech'));
    
    for (const exp of aExps) {
      const runs = await fetchRuns(exp.id);
      runs.forEach(r => {
        audioExperiments.push({
          expName: exp.name,
          run: r
        });
      });
  ...
  for (const item of audioExperiments) {
    const run = item.run;
  ...
  const oodChartData = latestRun && latestOODRun ? [
    { name: 'Baseline (In-Dist)', wer: metrics.test_wer, cer: metrics.test_cer },
    { name: 'OOD (Robustness)', wer: oodMetrics.test_wer, cer: oodMetrics.test_cer }
  ] : [];

  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Audio Model Performance</h1>
        <p className="text-muted-foreground mt-2">Speech Recognition (ASR) scaling metrics and failure analysis.</p>
      </div>
        ...
        <div className="mt-8">
          ...
          <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
            <CustomLineChart 
              ...
              xKey="name" 
              lines={[
                { key: 'wer', color: '#f59e0b', name: 'Word Error Rate' },
                { key: 'cer', color: '#10b981', name: 'Character Error Rate' }
              ]} 
            />
          </div>
        </div>
          ...
          {oodChartData.length > 0 && (
            <div className="bg-card border border-border rounded-xl p-6 shadow-sm mb-6">
              <CustomBarChart 
                ...
                xKey="name" 
                bars={[
                  { key: 'wer', color: '#f59e0b', name: 'WER' },
                  { key: 'cer', color: '#10b981', name: 'CER' }
                ]} 
              />
            </div>
          )}
