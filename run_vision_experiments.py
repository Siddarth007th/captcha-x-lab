...
def run_experiment(fraction):
    print(f"\n1. Creating Project for {fraction*100}% Vision Scale...")
    proj_resp = requests.post(f"{BASE_URL}/projects/", json={"name": f"Phase 9 Vision", "description": "Scaling experiments"})
    proj_id = proj_resp.json().get("id") or 1 # Fallback if already exists
    
    print(f"2. Creating Experiment in Project {proj_id}...")
    exp_resp = requests.post(f"{BASE_URL}/projects/{proj_id}/experiments/", json={"name": f"Vision-Scale-{fraction*100}pct"})
    ...
    run_resp = requests.post(f"{BASE_URL}/experiments/{exp_id}/runs", json={
        "model_version_id": 1,
        "mixture_config": {
            "model_architecture": "vision",
            "train_fraction": fraction,
            "num_epochs": 1,
            "batch_size": 2, # Small batch for max stability
